...
export default function WalletOverviewPage() {
  ...
  const calculateAnalytics = useCallback(() => {
    ...
    const totalValue =
      accountBalance.sol + tokenBalances.reduce((sum, token) => sum + (token.usdValue || 0), 0);
  ...
  const loadWalletData = useCallback(
    async (address: string) => {
      ...
      try {
        ...
        const updatedRecent = [address, ...recentAddresses.filter(addr => addr !== address)].slice(
          0,
          10,
        );
    ...
    <ToolLayout toolId='web3-wallet-overview'>
      ...
      >
                ...
                <CardDescription>
                  {walletAddress && (
                    <div className='flex items-center gap-2'>
                      ...
                      <Button variant='ghost' size='sm' onClick={() => copyAddress(walletAddress)}>
        ...
        <ActionButtons
          ...
          downloadFilename={
            accountBalance ? `wallet-portfolio-${walletAddress.slice(0, 8)}.json` : undefined
          }
