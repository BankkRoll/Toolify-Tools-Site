...
export default function AddressValidatorPage() {
  ...
  const validateBatchAddresses = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const results = await Promise.all(addresses.map(addr => validateAddress(addr)));
  ...
  return (
    <ToolLayout toolId='web3-address-validator'>
      ...
      <ProcessingStatus isProcessing={isLoading} isComplete={false} error={null} />
      ...
      >
        ...
        <ActionButtons
          ...
          downloadFilename={
            validationResults.length > 0 ? `address-validation-${Date.now()}.json` : undefined
          }
