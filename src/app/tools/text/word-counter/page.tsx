...
export default function WordCounterPage() {
  ...
  const [history, setHistory] = useLocalStorage<string[]>('text-word-counter-history', []);
  ...
  const stats = useMemo((): TextStats => {
    ...
    if (text.trim() && words > 0) {
      setHistory([`Analyzed text: ${words} words, ${characters} chars`, ...history].slice(0, 10));
    }
  ...
  const getReadabilityScore = (avgWordsPerSentence: number) => {
    if (avgWordsPerSentence <= 15) return { score: 'Easy', color: 'bg-green-500' };
    if (avgWordsPerSentence <= 20) return { score: 'Moderate', color: 'bg-yellow-500' };
      ...
      >
        ...
        <div className='space-y-6'>
          <MotionDiv
            ...
            variants={animationsEnabled ? cardVariants : undefined}
            initial={animationsEnabled ? 'hidden' : undefined}
            animate={animationsEnabled ? (statsSectionInView ? 'visible' : 'hidden') : undefined}
          >
            <Card>
              <CardHeader>
                <CardTitle className='flex items-center gap-2'>
          ...
          <MotionDiv
            ...
            variants={animationsEnabled ? cardVariants : undefined}
            initial={animationsEnabled ? 'hidden' : undefined}
            animate={animationsEnabled ? (readingSectionInView ? 'visible' : 'hidden') : undefined}
          >
            <Card>
              <CardHeader>
                <CardTitle className='flex items-center gap-2'>
              ...
              <CardContent className='space-y-4'>
                <div className='space-y-2'>
                  ...
                  <p className='text-xs text-muted-foreground'>Based on 200 words/minute</p>
                </div>
          ...
          <MotionDiv
            ...
            initial={animationsEnabled ? 'hidden' : undefined}
            animate={
              animationsEnabled ? (readabilitySectionInView ? 'visible' : 'hidden') : undefined
            }
          >
            <Card>
              <CardHeader>
          ...
          {stats.topWords.length > 0 && (
            <MotionDiv
              ...
              initial={animationsEnabled ? 'hidden' : undefined}
              animate={
                animationsEnabled ? (topWordsSectionInView ? 'visible' : 'hidden') : undefined
              }
            >
              <Card>
                <CardHeader>
