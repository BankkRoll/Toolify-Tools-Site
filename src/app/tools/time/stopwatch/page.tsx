...
export default function StopwatchPage() {
  ...
  const [history, setHistory] = useLocalStorage<string[]>('time-stopwatch-history', []);
  ...
  const addLap = () => {
    ...
    setHistory([`Lap ${lapCounter}: ${formatTime(newLap.lapTime)}`, ...history].slice(0, 10));
  };
  ...
  const getCsvContent = () => {
    ...
    const rows = laps
      .map(lap => `${lap.id},${formatTime(lap.time)},${formatTime(lap.lapTime)}`)
      .join('\n');
    return headers + rows;
  };
    ...
    <ToolLayout toolId='time-stopwatch'>
      ...
      >
        <MotionDiv
          ...
          initial={animationsEnabled ? 'hidden' : undefined}
          animate={animationsEnabled ? (timerSectionInView ? 'visible' : 'hidden') : undefined}
        >
          <Card>
            <CardHeader>
        ...
        <MotionDiv
          ...
          variants={animationsEnabled ? cardVariants : undefined}
          initial={animationsEnabled ? 'hidden' : undefined}
          animate={animationsEnabled ? (controlsSectionInView ? 'visible' : 'hidden') : undefined}
        >
          <Card>
            <CardHeader>
            ...
            <CardContent className='space-y-4'>
              <div className='flex gap-2'>
                <Button onClick={addLap} disabled={!isRunning} className='flex-1'>
                  <Square className='h-4 w-4 mr-2' />
                  Lap
                </Button>
                <ActionButtons
                  downloadData={new Blob([getCsvContent()], { type: 'text/csv' })}
                ...
                >
                  {getBestLap() && (
                    <div className='p-3 bg-green-50 border border-green-200 rounded-lg'>
                      <div className='text-sm font-medium text-green-800'>Best Lap</div>
                  ...
                  {getWorstLap() && (
                    <div className='p-3 bg-red-50 border border-red-200 rounded-lg'>
                      <div className='text-sm font-medium text-red-800'>Worst Lap</div>
        ...
        <MotionDiv
          ...
          variants={animationsEnabled ? cardVariants : undefined}
          initial={animationsEnabled ? 'hidden' : undefined}
          animate={animationsEnabled ? (lapsSectionInView ? 'visible' : 'hidden') : undefined}
        >
          <Card>
            <CardHeader>
              <CardTitle>Lap Times</CardTitle>
              <CardDescription>
                {laps.length > 0 ? `${laps.length} laps recorded` : 'No laps recorded yet'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {laps.length > 0 ? (
