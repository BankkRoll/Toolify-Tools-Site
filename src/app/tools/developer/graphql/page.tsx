...
export default function GraphqlPage() {
  ...
  const validateGraphql = (
    ...
    if (!hasQuery && !hasMutation && !hasSubscription) {
      errors.push(
        "Query must contain 'query', 'mutation', or 'subscription' keyword",
      );
    }
  ...
  const extractQueryInfo = (query: string) => {
    const operationMatch = query.match(
      /\b(query|mutation|subscription)\s+(\w+)/i,
    );
  ...
  const processGraphql = () => {
    ...
    try {
      ...
      const newEntry = {
        input:
          inputGraphql.length > 100
            ? inputGraphql.substring(0, 100) + "..."
            : inputGraphql,
        operation,
        timestamp: Date.now(),
      };
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to process GraphQL";
        ...
        >
          ...
          <Tabs defaultValue="input" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger
                value="output"
                disabled={!formattedGraphql && !validationResult}
              >
                Result
              </TabsTrigger>
            </TabsList>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <Badge variant="outline">
                              {validationResult.operationName}
                            </Badge>
                          </div>
                        </div>
                        ...
                        {validationResult.variableCount > 0 && (
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {validationResult.variables.map(
