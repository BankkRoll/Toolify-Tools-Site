...
export default function YamlPage() {
  ...
  const formatYaml = (yaml: string, indent: string): string => {
    ...
    for (const line of lines) {
      ...
      if (trimmed.includes(':')) {
        ...
        if (value) {
          formattedLines.push(indentStr.repeat(currentIndent) + `${key}: ${value}`);
        } else {
   ...
   */
  const validateYaml = (yaml: string): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    const lines = yaml.split('\n');
  ...
  const processYaml = () => {
    ...
    try {
      ...
      // Add to history
      const newEntry = {
        input: inputYaml.length > 100 ? inputYaml.substring(0, 100) + '...' : inputYaml,
        operation,
        timestamp: Date.now(),
      };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to process YAML';
        ...
        >
          ...
          <Tabs defaultValue='input' className='w-full'>
            <TabsList className='grid w-full grid-cols-2'>
              ...
              <TabsTrigger value='output' disabled={!formattedYaml && !validationResult}>
                Result
              </TabsTrigger>
            </TabsList>
