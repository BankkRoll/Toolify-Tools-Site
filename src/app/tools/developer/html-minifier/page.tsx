...
export default function HtmlMinifierPage() {
  ...
  const minifyHtml = (html: string, level: string): string => {
    ...
    if (level === "extreme") {
      ...
      minified = minified.replace(
        /<\/(p|li|dt|dd|tr|td|th|thead|tbody|tfoot)>/g,
        "",
      );
  ...
  const minifyHtmlInput = () => {
    ...
    try {
      ...
      const newEntry = {
        input:
          inputHtml.length > 100
            ? inputHtml.substring(0, 100) + "..."
            : inputHtml,
        output:
      ...
      toast.success(
        `HTML minified successfully! Size reduced by ${reduction}%`,
      );
    } catch (error) {
      const errorMessage =
