...
export default function FlexboxPage() {
  ...
  const [flexDirection, setFlexDirection] = useLocalStorage(
    "flexbox-direction",
    "row",
  );
  const [justifyContent, setJustifyContent] = useLocalStorage(
    "flexbox-justify",
    "flex-start",
  );
  const [alignItems, setAlignItems] = useLocalStorage(
    "flexbox-align",
    "stretch",
  );
  ...
  const [history, setHistory] = useLocalStorage<
    Array<{ css: string; timestamp: number }>
  >("flexbox-history", []);
...
.flex-item {
  ...
  font-weight: bold;
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to generate CSS";
