...
export default function XmlFormatterPage() {
  ...
  const [operation, setOperation] = useLocalStorage('xml-formatter-operation', 'format');
  const [indentSize, setIndentSize] = useLocalStorage('xml-formatter-indent', '2');
  ...
  const processXml = () => {
    ...
    try {
      ...
      if (operation === 'format') {
        ...
        result = validation.isValid ? 'XML is valid' : `XML has ${validation.errors.length} errors`;
      ...
      // Add to history
      const newEntry = {
        input: inputXml.length > 100 ? inputXml.substring(0, 100) + '...' : inputXml,
        operation,
        timestamp: Date.now(),
      };
        ...
        >
          ...
          <Tabs defaultValue='input' className='w-full'>
            <TabsList className='grid w-full grid-cols-2'>
              ...
              <TabsTrigger value='output' disabled={!formattedXml && !validationResult}>
                Result
              </TabsTrigger>
            </TabsList>
              ...
              {formattedXml && (
                <div className='space-y-2'>
                  <div className='flex items-center justify-between'>
                    <Label>Result</Label>
                    <button onClick={copyXml} className='text-sm text-blue-600 hover:text-blue-800'>
                      Copy
                    </button>
                  </div>
                  <Textarea
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className='flex items-center gap-2'>
                      {operation === 'validate' ? 'Validation Result' : 'XML Analysis'}
                      {operation === 'validate' && (
                      ...
                      <div className='space-y-4'>
                        <div className='grid grid-cols-2 gap-4'>
                          <div>
                            ...
                            <div className='flex flex-wrap gap-1'>
                              {validationResult.tags.map((tag: string, index: number) => (
                          ...
                          <div>
                            ...
                            <div className='flex flex-wrap gap-1'>
                              {validationResult.attributes.map((attr: string, index: number) => (
                        ...
                        <div className='flex gap-4'>
                          <div className='flex items-center gap-2'>
                            <Badge
                              variant={validationResult.hasDeclaration ? 'default' : 'secondary'}
                            >
                          ...
                          <div className='flex items-center gap-2'>
                            <Badge variant={validationResult.hasRoot ? 'default' : 'secondary'}>
                              {validationResult.hasRoot ? 'Has Root' : 'No Root'}
                            </Badge>
                          </div>
                        </div>
                      </div>
