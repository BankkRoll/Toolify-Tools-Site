...
export default function BaseConverterPage() {
  ...
  const [history, setHistory] = useLocalStorage<string[]>('number-base-converter-history', []);
   ...
   */
  const convertToBase = (value: string, fromBase: number, toBase: number): string => {
    try {
  ...
  const handleInputChange = (value: string) => {
    ...
    if (!isValidForBase(value, base)) {
      ...
      if (value.trim() && isValidForBase(value, base)) {
        const baseName = bases.find(b => b.value === inputBase)?.name || 'Unknown';
        setHistory([`Converted: ${value} (${baseName})`, ...history].slice(0, 10));
      }
    }
  };
    ...
    <ToolLayout toolId='number-base-converter'>
      ...
      >
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>Enter a number to convert between different bases</CardDescription>
            </CardHeader>
                ...
                <CardHeader className='pb-3'>
                  <div className='flex items-center justify-between'>
                    ...
                    <ActionButtons
                      ...
                      variant='outline'
                      size='sm'
                      disabled={!conversion.display || conversion.display === conversion.prefix}
                    />
                  </div>
        ...
        <MotionDiv
          ...
          variants={animationsEnabled ? cardVariants : undefined}
          initial={animationsEnabled ? 'hidden' : undefined}
          animate={animationsEnabled ? (examplesSectionInView ? 'visible' : 'hidden') : undefined}
        >
          <Card>
            <CardHeader>
