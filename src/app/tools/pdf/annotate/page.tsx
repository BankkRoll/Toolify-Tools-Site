...
export default function PdfAnnotatePage() {
  ...
  const [history, setHistory] = useLocalStorage<string[]>('pdf-annotate-history', []);
  ...
  const annotatePdf = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to annotate PDF';
      ...
      >
        ...
        {selectedFile && annotations.length > 0 && (
          <MotionDiv
            ...
            variants={animationsEnabled ? cardVariants : undefined}
            initial={animationsEnabled ? 'hidden' : undefined}
            animate={animationsEnabled ? (annotateSectionInView ? 'visible' : 'hidden') : undefined}
          >
            <Card>
              <CardHeader>
                <CardTitle className='flex items-center gap-2'>
                ...
                <CardDescription>Enter annotation text for each page (optional)</CardDescription>
              </CardHeader>
          ...
          >
            <Card>
              <CardHeader>
                ...
                <CardDescription>Download your annotated PDF file</CardDescription>
              </CardHeader>
              <CardContent>
                <ActionButtons
                  downloadData={annotatedPdf}
                  downloadFilename={
                    selectedFile ? `annotated-${selectedFile.name}` : 'annotated.pdf'
                  }
