...
export function ToolCard({
  ...
  const isRecentlyViewed = showRecentlyViewed && recentlyViewed.includes(tool.id);
              ...
              >
                <Star
                  className={`h-4 w-4 transition-all ${
                    isFavorite(tool.id) ? 'fill-current scale-110' : 'hover:scale-110'
                  }`}
                />
              </Button>
              {isRecentlyViewed && (
        ...
        <CardContent className={viewMode === 'list' ? 'flex-1' : ''}>
          ...
          <Button
            ...
            className={`w-full transition-all duration-200 ${
              tool.status === 'active' ? 'hover:shadow-md' : 'opacity-75 cursor-not-allowed'
            }`}
